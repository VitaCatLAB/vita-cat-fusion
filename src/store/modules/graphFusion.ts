...
import { CanvasLayerEnum, CanvasNameEnum } from '@/graph-schema';
...
export const useGraphFusionStore = defineStore({
  id: 'app-graph-fusion',
  persist: createPersistConfig('app-graph-fusion'),
  ...
  getters: {
    // 获取当前激活画布的详细信息
    activeCanvasInfo(state): CanvasInfo | null {
      return state.activeCanvas ? state.canvasMap[state.activeCanvas] : null;
    },
  ...
  actions: {
    ...
    // },
    /** 修改画布尺寸 */
    updateCanvasSize(name: CanvasNameEnum, size: { width: number; height: number }) {
      const canvas = this.canvasMap[name];
      if (canvas) {
    ...
    updateCanvasOffset(name: CanvasNameEnum, offset: { x?: number; y?: number }) {
      const canvas = this.canvasMap[name];
      if (!canvas) return;
      // 判断 offset 是否为空对象
...
export function useGraphFusionStoreWithOut() {
  return useGraphFusionStore(store);
}
// const store = useGraphFusionStore()

// // 创建一个画布
// store.createCanvas('编辑器画布', { width: 800, height: 600 })
// store.createCanvas('轮廓线画布', { width: 1024, height: 768 })

// // 切换画布（通过名字）
// store.setActiveCanvasByName('编辑器画布')

// // 获取当前激活的画布信息
// const activeCanvas = store.canvasMap[store.activeCanvas ?? '']

// // 更新画布尺寸
// store.updateCanvasSize('编辑器画布', { width: 1200, height: 900 })

// // 删除画布
// store.removeCanvasByName('轮廓线画布')
