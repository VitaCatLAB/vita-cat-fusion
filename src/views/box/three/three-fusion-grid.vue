...
<script setup lang="ts">
  ...
  import { create3DBoxGrid, initLabelRenderer } from './three-coordinate-axis';
  import './three.css';

  interface AxisConfig {
    axisLength: number; // 轴长度（±值）
    tickStep: number; // 刻度间距
    showLabels?: boolean; // 是否显示刻度数值标签
    showAxisNames?: boolean; // 是否显示 X/Y/Z 轴文字
  ...
  const initScene = () => {
    ...
    labelRenderer = initLabelRenderer(containerRef.value!);
  };
  ...
  const initControls = () => {
    ...
    // arcballControls = new ArcballControls(currentCamera, renderer.domElement);
    // arcballControls.enabled = true;
    // arcballControls.enableAnimations = true;
    // arcballControls.enableGrid = true;
  };
  ...
  const addSphere = () => {
    ...
    create3DBoxGrid({
      ...
      steps: { x: 10, y: 10, z: 10 },
      labels: { x: 'carbohydrate', y: 'calcium', z: 'sodium' },
      scene,
      fontSize: 16,
    });
  };
