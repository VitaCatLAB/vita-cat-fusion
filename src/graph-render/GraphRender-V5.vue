...
<script lang="ts" setup>
   ...
   * 图层生成函数
   * ========================= */
  const generateTestLayerObjects = async (): Promise<fabric.Object[]> => {
   ...
   * - 若不传，则默认重建画布配置中的全部图层
   */
  const resetCanvasLayers = async (baseLayers: CanvasLayerEnum[] | CanvasLayerEnum = []) => {
    /**
     * 图层生成器映射表
     *
     * Key   ：图层枚举值
     * Value ：返回 Promise<fabric.Object[]> 的异步生成函数
     *
     * 设计目的：
     * - 不同图层拥有独立的“生成策略”
     * - resetCanvasLayers 本身不关心图层如何生成
     */
    const layerGenerators: Partial<Record<CanvasLayerEnum, () => Promise<fabric.Object[]>>> =
    ...
    const staleLayers = existingLayers.filter(
      (layer) => !targetLayers.includes(layer as CanvasLayerEnum),
    );
     ...
     */
    const layersToRebuild = sourceLayers.filter((layer) => !excludedLayerList.includes(layer));
   ...
   * Canvas 鼠标事件
   * ========================= */
  const handleCanvasMouseDown = (opt: fabric.IEvent) => {
  ...
  const handleCanvasMouseMove = (opt: fabric.IEvent) => {
  ...
  const updateCanvasState = (
    ...
    if (mode === 'create') {
      graphFusionStore.createCanvas(editorCanvasConfig.name, editorCanvasConfig.layer, size);
  ...
  const canvasTriggerUUID = computed(() => {
    return graphFusionStore?.canvasMap[editorCanvasConfig.name]?.triggerUUID || '';
  });
  /* =========================
   * 监听触发器
   * ========================= */
  ...
  onUnmounted(() => {
    ...
    window.removeEventListener('resize', () => updateCanvasState('update', 'size'));
