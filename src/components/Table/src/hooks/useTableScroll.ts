...
import { useWindowSizeFn, onMountedOrActivated } from '@vita/hooks';
...
export function useTableScroll(
   ...
   * @description 来自于 .vita-basic-table .ant-table-wrapper
   */
  ...
  function calcBottomAndPaddingHeight(tableEl: Element, headEl: Element) {
    ...
    if (unref(wrapRef) && isCanResizeParent) {
      ...
      if (formHeight) {
        // 来自于 .vita-basic-table-form-container .ant-form 以及 .vita-basic-table-form-container
        formHeight += 16 + 16 * 2;
      }
  ...
  function getMarginPaddingHeight() {
    ...
    return (
      tableWrapperPadding + 16 // 来自于 .vita-basic-table-form-container 或是 .p-4
    );
  }
